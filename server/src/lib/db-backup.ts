...
import { isPostgresTarget, pgVaultRead, pgVaultWrite, redactPostgresUrl } from './pg-vault.js';
...
function backupTarget(): string | null {
  ...
  // Postgres vault fallback: when the host only offers Postgres (ephemeral
  // filesystem + managed Postgres, e.g. Hostless), setting DATABASE_URL alone
  // is enough to persist the encrypted SQLite dump across redeploys.
  // See server/src/lib/pg-vault.ts.
  const pgUrl = (process.env.DATABASE_URL ?? process.env.POSTGRES_URL ?? '').trim();
  if (pgUrl && isPostgresTarget(pgUrl)) return pgUrl;
  return null;
}

/** Display-safe target for logs — never prints a Postgres password. */
function displayTarget(target: string): string {
  return isPostgresTarget(target) ? `postgres:${redactPostgresUrl(target)}` : target;
}
...
async function readTarget(target: string): Promise<Buffer | null> {
  if (isPostgresTarget(target)) return pgVaultRead(target);
...
async function writeTarget(target: string, payload: Buffer): Promise<void> {
  if (isPostgresTarget(target)) {
    await pgVaultWrite(target, payload);
    return;
  }
...
export async function restoreDbBackupIfNeeded(dbPath = getDefaultDbPath()): Promise<DbBackupResult> {
  ...
  try {
    ...
    throw new Error(`could not restore SQLite backup from ${displayTarget(target)}: ${detail}`);
  }
  ...
  console.log(`[db-backup] restored ${restored.length} bytes from ${displayTarget(target)}`);
...
export async function backupDbNow(db: Db, dbPath = getDefaultDbPath()): Promise<DbBackupResult> {
  ...
  console.log(`[db-backup] uploaded ${plain.length} bytes to ${displayTarget(target)}`);
